...
@router.get("/download_split_wafer_csv")
async def download_split_wafer_csv(db: Session = Depends(get_db)):
    query_result = db.query(SWLY_LABEL_LIST).all()
    ...
    return StreamingResponse(generate_csv(), media_type="text/csv", headers={"Content-Disposition": "attachment; filename=swly_label_list_split_wafer.csv"})